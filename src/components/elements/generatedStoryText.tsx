...
export const GeneratedStoryText = (props: any) => {
  ...
  const ConstructTranslationSentence = (
    appears_in_french: boolean,
    appears_in_amharic: boolean,
    appears_in_latin: boolean,
    appears_in_italian: boolean,
    ...
    if (
      ...
      english_translation.toLowerCase() != "summary only"
    ) {
  ...
  const ConstructReadAloudSentence = (readings_date: string) => {
    if (readings_date != null) {
      return (
        <>
          This story is assigned by to <u>read aloud</u> on a particular day
          during the Ethiopian church service: it is read on a {readings_date}.{" "}
        </>
      );
    }
    return;
  ...
  const ConstructLifeMiracleSentence = (type_of_story: string) => {
    ...
    return (
      <>
        This story is a <u>post-life miracle</u>: it does not take place during
        Our Lady Mary&apos;s lifetime, but after it.
      </>
    );
  };

  // function to write story setting sentence, if any
  const ConstructStoryPlaceSentence = (
    story_place: string,
    story_place_type: string
  ) => {
    if (story_place != null) {
      return (
        <>
          This story takes place in {story_place}{" "}
          {story_place_type != null ? story_place_type : ""}.{" "}
        </>
      );
    }
    return;
  ...
  return (
    <>
      ...
      <div style={{ marginTop: "10px", textIndent: "20px" }}>
        <small>
          <sup>1</sup> A &quot;PEMM manuscript&quot; is defined as any Gəˁəz
          Marian manuscript that PEMM has catalogued. For more information, see
          About page.
        </small>
      </div>
    </>
